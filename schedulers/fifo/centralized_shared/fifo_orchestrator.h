...
class FifoOrchestrator {
 public:
  typedef std::function<void(FifoOrchestrator& orch,
  ...
  // Prints all cached FifoSchedParams.
  ...
  inline bool Repeating(const FifoSchedParams* sp) {
  ...
  // Marks an engine runnable, both in the cached 'FifoSchedParams'
  // corresponding to the engine along with in the live PrioTable sched item
  // corresponding to the engine.
  void MakeEngineRunnable(const FifoSchedParams* sp);
 ...
 private:
  ...
  std::unique_ptr<FifoSchedParams[]> cachedsids_;
};
